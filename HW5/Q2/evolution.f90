...
subroutine flux(i,dt,FL,FR)
    ...
    !The Lax-Friedrichs
    !FL=0.5*c*(uold(i-1)+uold(i))-dx*(uold(i)-uold(i-1))/(2.0*dt)
    !FR=0.5*c*(uold(i)+uold(i+1))-dx*(uold(i+1)-uold(i))/(2.0*dt)
    !The Lax-Wendroff
    FL=c*(0.5*(uold(i-1)+uold(i)-0.5*dt*c*(uold(i)-uold(i-1))/dx))
    FR=c*(0.5*(uold(i)+uold(i+1)-0.5*dt*c*(uold(i+1)-uold(i))/dx))
